...
> 관련 API: `POST /report/individual`, `POST /report/daily`, `GET /report/individual/{id}`, `GET /report/daily/{id}`
> `report.ready` WS 이벤트 미확정 → 자동 갱신 없음. 생성 후 수동 `fetchDetail()` 재호출 필요.
